/**
 * Estonian Quote PDF Generator
 * 
 * Generates PDF quotes in the standard Estonian format (similar to invoice but labeled as "Hinnapakkumine")
 * 
...
import {
  ...
  generateViitenumber,
  calculateDueDate,
} from './estonianInvoice';

// Re-export shared utilities
export { DEFAULT_COMPANY, formatEstonianNumber, formatEstonianDate };

/**
 * Generate quote data structure
 */
export const createQuoteData = ({
  ...
  const valid = validUntil ? new Date(validUntil) : calculateDueDate(date, validDays);
...
/**
 * Generate HTML for Estonian quote (for PDF conversion or printing)
 */
export const generateQuoteHTML = (quoteData) => {
  const {
    quote_number,
    date,
    valid_until,
...
<head>
  ...
  <title>Hinnapakkumine ${quote_number}</title>
  <style>
    * {
      margin: 0;
      padding: 0;
...
<body>
  <div class="quote">
    <!-- Header: Recipient + Quote Meta -->
    <div class="header">
      <div class="recipient">
        <div class="quote-badge">HINNAPAKKUMINE</div>
        <div class="section-title">Saaja</div>
        ...
        ${recipient.email ? `<div class="reg-info">E-post: ${escapeHtml(recipient.email)}</div>` : ''}
        ${recipient.phone ? `<div class="reg-info">Tel: ${escapeHtml(recipient.phone)}</div>` : ''}
      </div>
      ...
      <div class="quote-meta">
        <table class="meta-table">
          <tr>
            <td>Pakkumine nr</td>
            <td class="quote-number">${escapeHtml(quote_number)}</td>
          </tr>
          <tr>
            <td>Kuupäev</td>
          ...
          <tr>
            <td>Kehtiv kuni</td>
            <td>${formatEstonianDate(valid_until)}</td>
          </tr>
        </table>
      </div>
    </div>
    ...
    <div class="validity-notice">
      <strong>NB!</strong> Käesolev hinnapakkumine kehtib kuni ${formatEstonianDate(valid_until)}. 
      Tellimuse kinnitamiseks palume võtta ühendust.
    </div>
...
/**
 * Open quote in new window for printing/saving as PDF
 */
export const printQuote = (quoteData) => {
 ...
 */
export const downloadQuoteHTML = (quoteData) => {
  const html = generateQuoteHTML(quoteData);
  const blob = new Blob([html], { type: 'text/html' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `Hinnapakkumine_${quoteData.quote_number}.html`;
